...
<div class="col-md-12">
    ...
    @elseif(Session::get('status')=='IOT Status')
    <table id="dataListMetric" class="table table-bordered asd" width="100%">
        <thead>
            <tr id="theadsize">
                <th>Operator name</th>
                <th>Skema</th>
                @for($i = \Carbon\Carbon::now()->subYears(5)->year; $i <= \Carbon\Carbon::now()->year; $i++)
                    <th>{{ $i }}</th>
                @endfor
            </tr>
        </thead>
        <tbody>
            @inject('implement', 'App\Implement')
            @foreach($data as $listData)
            <tr>
                <td>{{$listData->operator->nama}}</td>
                <td>{{$listData->skema}}</td>
                @for($i = \Carbon\Carbon::now()->subYears(5)->year; $i <= \Carbon\Carbon::now()->year; $i++)
                    @php
                        $status = $implement->where('operator_id', $listData->operator_id)
                            ->where('skema', $listData->skema)
                            ->where('years', $i)
                            ->get()
                            ->first()
                    @endphp
